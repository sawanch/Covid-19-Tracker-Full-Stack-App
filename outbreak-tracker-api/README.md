# Respiratory Outbreak Monitoring API

A production-ready RESTful API backend built with Spring Boot that processes and serves respiratory disease outbreak statistics including COVID-19, Flu, RSV, and Pneumonia. The API aggregates province/state level data to country-level statistics and provides real-time access through standardized REST endpoints with early-warning trends and AI-powered insights.

## Overview

The Respiratory Outbreak Monitoring API is a Spring Boot application that transforms raw CSV data into structured, queryable information. It processes time-series outbreak data, performs country-level aggregation, calculates derived metrics, and serves the data through RESTful endpoints.
...
- **Real-Time Access**: Provides instant access to current outbreak statistics via HTTP
- **Analytics**: Tracks API usage patterns and request metrics for operational insights
- **AI Insights**: Provides intelligent recommendations and trend analysis for outbreak monitoring
...
│  • Johns Hopkins time-series outbreak data          │
...
- Run `OutbreakTrackerApiApplication.java` as a Java application
...
GET /api/global
```
Returns aggregated worldwide outbreak statistics by summing data from all countries.
...
The API performs ETL (Extract, Transform, Load) operations on time-series CSV data:

1. **Extract**: Reads CSV file with daily cumulative confirmed cases from data repository
2. **Transform**: 
...
│   ├── util/
│   │   └── CsvParserUtil.java                 
│   └── OutbreakTrackerApiApplication.java      # Main application class
│
├── src/main/resources/
...
**Sample Log Lines:**
```
2024-01-15 10:30:45 [main] INFO  com.covidtracker.api.OutbreakTrackerApiApplication - Starting OutbreakTrackerApiApplication
2024-01-15 10:30:47 [main] INFO  com.covidtracker.api.controller.CovidDataController - GET /api/global - Fetching global statistics
2024-01-15 10:30:48 [main] DEBUG com.covidtracker.api.service.impl.CovidDataServiceImpl - Fetching global statistics from database
```
